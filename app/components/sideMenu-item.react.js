  	...
  	render() {
		...
		for (var i = 0; i < data.length; i++) {
			if(data[i].href == "movies"){
				menus.push(<li key={i}><IndexLink to="/" activeClassName="active">{data[i].name}</IndexLink></li>)
			}else{
				menus.push(<li key={i}><Link to={"/" + data[i].href} activeClassName="active">{data[i].name}</Link></li>);
			}
		};
